...
bool ImGui::Checkbox(const char* label, bool* v, bool lever)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    BeginChildFrame(id, frame_bb.GetSize(), extra_flags);
    return true;
}
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginListBox(label, size, extra_flags))
        return false;
